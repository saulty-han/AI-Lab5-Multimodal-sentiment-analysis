...
from multi_image_only_model import MultimodalSentimentAnalysisModel_atten,MultimodalSentimentAnalysisModel_cat_trans,MultimodalSentimentAnalysisModel_add,MultimodalSentimentAnalysisModel_cat_direct
import argparse

def parse_arge():
    parse=argparse.ArgumentParser(description='add parameter')
    parse.add_argument('--model',default='cat_trans',help='cat_direct or add or cat_trans or cat_atten')
...
if args.model=='cat_direct':
    model = MultimodalSentimentAnalysisModel_cat_direct(resnet_model, num_labels=3, hidden_dim=args.hidden_dim)
elif args.model=='add':
    model = MultimodalSentimentAnalysisModel_add(resnet_model, num_labels=3, hidden_dim=args.hidden_dim)
elif args.model=='cat_trans':
    model = MultimodalSentimentAnalysisModel_cat_trans(resnet_model, num_labels=3, hidden_dim=args.hidden_dim)
elif args.model=='cat_atten':
    model = MultimodalSentimentAnalysisModel_atten(resnet_model, num_labels=3, hidden_dim=args.hidden_dim, num_heads=args.num_heads, dropout=args.dropout)
